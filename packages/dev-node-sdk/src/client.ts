...
export class PlayerDataClient {
  ...
  private async initializeUserId(): Promise<void> {
    ...
    try {
      const response = await fetch(
        `${this.baseUrl}${CONFIG.ENDPOINTS.USER_ID}`,
      );
      ...
      if (!this.userId) {
        throw new GameSDKError("No user ID returned from server", "AUTH_ERROR");
      }
    } catch (error) {
  ...
  async sendPlayerData<T>(data: T): Promise<void> {
    ...
    try {
      const response = await fetch(
        `${this.baseUrl}${CONFIG.ENDPOINTS.PLAYER_DATA}`,
        {
          method: "POST",
          credentials: "include",
          headers: {
  ...
  async getGameData<T>(): Promise<T[]> {
    ...
    if (this.devMode) {
      console.log(
        "[DEV MODE] Would fetch all game data for game:",
        this.gameId,
      );
      return [] as T[];
    }
    ...
    try {
      ...
      const result = await response.json();
      const dataArray =
        result.data?.map((item: PlayerDataPayload) => item.data) || [];
